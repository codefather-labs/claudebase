---
name: access
description: Manage claudebase channel access — approve pairings, edit allowlists, set DM/group policy. Use when the user asks to pair, approve someone, check who's allowed, or change policy for the claudebase Telegram channel.
user-invocable: true
allowed-tools:
  - Read
  - Write
...
1. Read `~/.claude/channels/claudebase/access.json`.
...
7. `mkdir -p ~/.claude/channels/claudebase/approved` then write
   `~/.claude/channels/claudebase/approved/<senderId>` with `chatId` as the
   file contents. The daemon polls this dir and sends "you're in".
...
- The channels dir might not exist if the daemon hasn't run yet — handle
