...
## §22. HTTP Callback API — external systems write into a session's input

**Status:** [SHIPPED]
**Date:** 2026-08-17
**Priority:** Medium
**Related:** §21 (PTY transport — this reuses its injection gate and delivery path wholesale; the
callback adds an INPUT, not a transport), §20 (session-to-session routing — the same
`send_message` + bus publish path carries callbacks). Design source:
`docs/plans/claudebase-v0.11-callback-api.md`.

Changelog: A CI job, webhook or script can POST to the daemon and have the body appear in a named
session's terminal input.

Use cases: [`docs/use-cases/callback-api_use_cases.md`](use-cases/callback-api_use_cases.md).

### 22.1 Feature Description

Telegram reaches a session, and peer sessions reach each other, but nothing outside either channel
could. The operator's case is event-driven harness debugging: something happens elsewhere — a build
fails, a watchdog fires — and the session should hear about it, eventually from another machine.

`POST http://<bind>/callback/<nick>?label=<label>` with an `X-Api-Token` header. The body arrives as
`[callback:<label>]: <text>`.

### 22.2 Functional Requirements

**FR-CB-1 (Off by default).** The daemon MUST NOT listen until the operator enables it. Tokens exist
from a session's first registration; opening a port is a separate act.

**FR-CB-2 (Authentication is mandatory).** Requests MUST present `X-Api-Token`. There MUST be no
flag disabling authentication. Comparison MUST be constant-time.

**FR-CB-3 (Tokens are per-nick and outlive the process).** A token MUST be keyed by nick, never by
`agent_id`, which is re-minted every restart and would break any script embedding it. A rename MUST
carry the token to the new nick by default — a rename usually means the window is called something
else, not that it is a different session, and reissuing silently would break every working
integration on what the operator considers cosmetic. A rename MUST also offer the opposite
(`--new-callback-token`), retiring the old token in the same step, for when the rename marks a change
of purpose or of hands and the previous holder should lose access. A token MUST authorise only its
own target.

**FR-CB-4 (The secret is generated by the daemon).** From the OS CSPRNG. It MAY be displayed to the
operator by an agent — accepted risk, see Decisions — but MUST NOT be generated by one.

**FR-CB-5 (Loopback by default).** Binding a non-loopback address MUST require an explicit opt-out
flag and MUST warn that the token travels in cleartext over plain HTTP.

**FR-CB-6 (Always 200, verdict in the body).** The HTTP status MUST be `200` regardless of outcome;
the body MUST distinguish acceptance from rejection and name the reason, so a mistyped nick is not
indistinguishable from a delivery.

**FR-CB-7 (Own prefix, optional label).** Inbound text MUST be rendered `[callback]:` or
`[callback:<label>]:` and MUST NOT reuse another channel's prefix. The label MUST be sanitised with
the same rule as nicks, since it is pasted inside the prefix.

**FR-CB-8 (Control characters stripped).** Request bodies MUST have control characters other than
newline and tab removed before injection: the body is pasted into a pty.

**FR-CB-9 (Source is stated, not inferred).** The notification MUST carry an explicit source marker.
Inferring the channel from the shape of other fields is what produced F-14 in §21.

### 22.4 The UNIX-socket surface

**FR-CB-10 (One socket per live agent).** The daemon MUST expose
`$XDG_RUNTIME_DIR/claudebase/agents/<nick>.sock` for every alive agent, create it
without the operator enabling anything, remove it when the agent goes, and move
it on a rename. Reconciliation MUST be periodic rather than event-driven, so the
set of sockets self-heals after a crash or a missed disconnect.

**FR-CB-11 (No token).** The socket MUST NOT require authentication. Its
directory is 0700 and the socket 0600, so the kernel already restricts it to the
owning user; a token would guard what is guarded and add a secret to store, show
and rotate. This is the opposite of FR-CB-2 for a reason: a TCP port is reachable
by anyone who can route to it, a socket here is not.

**FR-CB-12 (Connection close is the message boundary).** One connection carries
exactly one message, ending at EOF. No length prefix, no delimiter. The daemon
MUST answer with the same JSON verdict shape the HTTP surface uses.

**FR-CB-13 (Shared delivery).** Both surfaces MUST use one delivery function, so
the injection gate, storage and notification shape cannot drift apart between
them.

**FR-CB-14 (Same body rules).** `{"text","label"}` unwrapping, label sanitising
and control-character stripping MUST be identical to the HTTP surface.

### 22.3 Out of Scope (v1, deliberate)

Rate limiting, `Origin`/`Host` checks, a body-size ceiling beyond the transport minimum,
`?wait=delivered`, TLS, and a synchronous agent reply. All are recorded as debt owed by the
external-bind slice; see FR-CB-6.1 of the design doc.

## Facts

### Verified facts
- The endpoint reuses `send_message` + `bus.publish`, the same path peer messages take — source: `src/daemon/callback.rs::dispatch`, mirroring `handle_agent_send` at `src/daemon/server.rs:1705` — salience: high.
- A token issued to one nick is refused for another; verified live against a running daemon, not only in tests — source: `tests/callback_http_test.rs::a_token_does_not_open_another_session`, plus a live `curl` on 2026-08-17 returning `unauthorized` — salience: high.
- A rename moves the token: the fingerprint issued to `planner` appeared under `transport` after `agent rename`, and `planner` left the table — source: live `claudebase daemon callback status` before and after — salience: high.
- End to end on real HTTP: a POST produced `[callback:selftest]: …` in the session's input — source: observed live 2026-08-17 after restarting the session on the new binary — salience: high.
- The supervisor had no shutdown path (no signal handlers; stdin EOF discarded), so a closed terminal left `claude` running and its transcript unavailable to `/resume` — source: `src/supervisor/mod.rs:211` before the fix; reproduced by `tests/supervisor_shutdown_test.rs::a_child_that_ignores_hup_and_term_is_still_reaped`, which fails without it — salience: high.

### External contracts
- **HTTP/1.1** — symbol: request line, `Content-Length`, header folding not supported, `Connection: close` — source: the hand-written parser in `src/daemon/callback.rs::handle_connection`, exercised by `tests/callback_http_test.rs` against real sockets — verified: yes — salience: medium.
- **`getrandom` 0.2** — symbol: `getrandom::getrandom(&mut [u8])` — source: declared in `Cargo.toml`, called in `callback::mint` and exercised by tests asserting a 64-char hex token — verified: yes — salience: high.

### Assumptions
- Port 8585 is an acceptable default; the operator's stated port (`85855`) is not a valid TCP port and was never confirmed — risk: the operator expects a different default — how to verify: ask; it is one flag — salience: medium.

### Open questions
- Whether "no checks for now" was meant to include the deferred `Origin`/rate-limit protections permanently or only until the port is opened externally — needs: user decision before the external-bind slice — salience: high.

## Decisions

### Inbound validation
- The requested endpoint writes into a session running with permissions skipped, i.e. it is an RCE surface rather than a message channel. Challenged: yes. Outcome: built with authentication mandatory, loopback default, and the port off until enabled — salience: high.
- The operator asked that an agent generate the token. Challenged: yes — LLM output is not a CSPRNG. Outcome: generation stays in the daemon; display by an agent accepted — salience: high.
- The operator asked that an agent hand over the token in plain text, after the leak-into-transcript objection was raised twice. Outcome: **operator decision accepted and implemented**; risk compensated and logged rather than hidden — salience: high.

### Decisions made
- The socket surface carries no token, unlike HTTP. Alternatives rejected: reusing the per-nick token for symmetry — it would add a secret to guard what file permissions already guard, and every stored secret has to be shown and rotated. Q1-Q5: hack? no | sane? yes | alternatives? listed | cause | n/a — salience: high.
- Socket lifecycle is a reconciliation loop, not hooks on register/disconnect. Alternatives rejected: creating the socket in the register handler and removing it on EOF — shorter to write, but it gets a daemon crash, a rename and a missed disconnect wrong exactly once and then stays wrong. Q1-Q5: hack? no | sane? yes | alternatives? listed | cause | n/a — salience: medium.
- Documentation names `socat` and a socket library, NOT `nc -U`. Alternatives rejected: the obvious `nc -U` one-liner — measured to be denied by AppArmor on Ubuntu, so documenting it would have misled users on the most common distribution. Q1-Q5: hack? no | sane? yes | alternatives? listed | cause | n/a — salience: medium.
- Token keyed by nick alone, narrowed from the design's "nick + cwd" during implementation: the URL carries only a nick, so a table able to return two rows for one address would eventually return the wrong one. Q1-Q5: hack? no | sane? yes | alternatives? listed | cause | n/a — salience: high.
- A hand-written HTTP/1.1 handler for one route rather than adding axum. Alternatives rejected: axum (a new direct dependency in a process that listens on a socket, for one POST). Mitigation: no keep-alive state machine, connection closed every time, bounded head and body. Q1-Q5: hack? borderline — hand-rolled HTTP is a known trap, which is why the surface is one route and the parser refuses anything else | sane? yes | alternatives? listed | cause | n/a — salience: medium.
- An explicit `meta.source` on the callback notification instead of inferring the channel. Q1-Q5: hack? no | sane? yes | alternatives? inference — rejected, it is the direct cause of F-14 | cause | n/a — salience: high.
- `meta.from_agent` added purely so supervisors predating this feature degrade to `[agent-to-agent:callback-<label>]` rather than `[agent-to-agent:unknown]`. Q1-Q5: hack? no — compatibility, not a workaround | sane? yes, mixed binary versions are the normal case when sessions stay open for hours | alternatives? leaving it unreadable | symptom | n/a — salience: low.

### Hacks / workarounds acknowledged
- The callback token is displayed by an agent in plain text and therefore persists in the session transcript. Why it's a hack: a secret lands in a file that gets read, quoted and shared. Accepted on the operator's explicit, repeated instruction. Compensations: one token per session, loopback default, one-command rotation. Removal path: revert to file-only issuance with a fingerprint in stdout if an untrusted transcript reader ever becomes plausible — salience: high.
- Self-echo on the callback endpoint is prevented by an instruction in a skill, not by a mechanism. Why it's a hack: the endpoint cannot distinguish an agent's own curl from anyone else's. Removal path: daemon-side dedup if a loop is ever observed — salience: medium.

### Symptom-only patches (with root-cause links)
- Stripping control characters from request bodies treats the symptom (ESC reaching the pty); the root cause is that channel content is trusted by a model running with `--dangerously-skip-permissions`. Tracked at: risk R-6 of the v0.10 plan — salience: high.
