...
| TC-IHC-13.7 | UC-IHC-13-E2 | CLI | Open an `insights.db` with `PRAGMA user_version = 6` (unknown future version) | Exits 1 with stderr containing the unified corruption-gate message (the codebase maps every schema-validation failure to the same operator-facing line — verified at `main.rs:2320` / `2451`) | `assert_cmd.assert().failure().code(1)`; stderr contains literal substring `error: index database invalid; re-ingest required` |
...
| ID | Use Case | Verification Class | Test Case | Expected Result | Evidence Required |
|----|----------|--------------------|-----------|-----------------|--------------------|
| TC-IHC-16.1 | UC-IHC-16 | FS | AC-IHC-15 acceptance test: lift the new SessionStart jq wiring block out of `install.sh`, point it at a temp copy of `~/.claude/settings.json`, run it TWICE (do NOT run the full `install.sh --yes` against the live system — Slice 7 verify convention), inspect the temp settings.json | Exactly one entry for `claudebase-read-insights-reminder.sh` in the nested SessionStart structure | Claude Code's real SessionStart shape is nested `{matcher, hooks:[{type,command}]}` (NOT flat). Correct selector: `jq '[.hooks.SessionStart[]?\|.hooks[]?\|select(.command\|contains("claudebase-read-insights-reminder"))] \| length' <temp>` == 1 |
